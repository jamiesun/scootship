...
当前门禁状态：**关闭**。E1 侧现在已有生产 / dev 传输 fail-closed 行为、部署 / 恢复文档、审计保留与 gap 可见性、token 生命周期治理、运行审计时间线、只读健康信号、端点失败模式和严格遥测 body 校验的证据。E2 仍被阻塞，直到 Scoot 侧发布无人值守 readonly clamp 与兼容 edge 契约，并且 Scootship 中存在派发威胁模型以及派发专属代码和测试。
