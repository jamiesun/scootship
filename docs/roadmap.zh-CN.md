...
- **节点注册表、令牌鉴权与令牌生命周期。** 每节点独立 bearer token 鉴权，令牌只能为自身 `node_id` 说话；
  仪表盘操作员可以创建、轮换和撤销中心托管的节点认证令牌。仪表盘 / API 只展示安全的清单元数据（来源、
  fingerprint、最近认证时间），绝不显示 bearer secret（`internal/tokens`、`internal/center`、`internal/web`）。
...
阶段二（任务编排与派发）尚未实现：`GET /jobs/lease` 目前是观测期占位，按契约鉴权，校验节点与有界 capacity 请求，但不派发任何作业。
...
- **mock edge 测试装置。** 因为真实 `scoot-edge` 尚不存在，Scootship 必须能在没有真实边缘的情况下开发与验证：一个能模拟节点心跳、能力描述符与 `audit_batch` 上报的测试边缘。阶段一的 lease 轮询只验证已鉴权、绑定节点、capacity 有界的空派发契约（`X-Scootship-Dispatch: disabled-phase1`）；真实作业生命周期模拟属于 E2，必须等派发门禁满足后再做。
...
当前门禁状态：**关闭**。E1 侧现在已有直连 TLS、可信 TLS 反代 HTTP、显式 dev HTTP、默认明文 HTTP fail-closed 行为的代码与测试证据，也已有部署 / 恢复文档、审计保留与 gap 可见性、token 生命周期治理、运行审计时间线、只读健康信号、端点失败模式和严格遥测 body 校验的证据。预派发威胁模型位于 [`dispatch-threat-model.zh-CN.md`](dispatch-threat-model.zh-CN.md)，但其中假设仍需 owner 确认。E2 仍被阻塞，直到 Scoot 侧发布无人值守 readonly clamp 与兼容 edge 契约，并且 Scootship 中存在派发专属代码和测试。
