...
- **运维可信度先于远程编排。** 中心侧 E2 派发代码必须在明确门禁后生长：生产 / dev 传输边界要经过测试，部署与恢复要可复现，审计生命周期和 gap 要有实现，节点认证要可治理，健康信号要能在仪表盘上看清；在 edge 侧上线假设满足前，面向操作员的派发入口仍保持关闭。
...
当前门禁状态：**中心侧核心已部分打开；操作员派发仍需门禁**。E1 侧现在已有直连 TLS、可信 TLS 反代 HTTP、显式 dev HTTP、默认明文 HTTP fail-closed 行为的代码与测试证据，也已有部署 / 恢复文档、审计保留与 gap 可见性、token 生命周期治理、运行审计时间线、只读健康信号、端点失败模式和严格遥测 body 校验的证据。Scoot `main` 现在已把无人值守一次性钳制（`scoot -e --unattended`）作为 E2 关键前置写入并实现；同时它的 EDGE.md 仍把 E2 上线门控在 `edge.job_root` cwd confinement。Scootship 现在已有派发队列、lease、幂等、能力不匹配拒绝与生命周期测试。剩余 edge 侧假设和操作员文档满足前，面向操作员的派发 UI/API 仍被阻塞。
...
- **派发溯源审计。** 已部分实现：中心侧记录派发 provenance 并更新 lifecycle/session 关联；后续可通过
  `session_id` 关联回摄入的 Scoot 运行审计，形成端到端可追溯链路。
- **E2 前置条件必须清楚。** 队列语义、能力 / 标签匹配、`idem_key` 幂等、capacity 限制、deadline、
  `job_event` 处理、派发溯源，以及 Scoot 侧无人值守 readonly clamp 现在已有代码、测试或上游契约证据；
  面向操作员的派发仍等待剩余 edge cwd confinement 和 runbook 工作。
