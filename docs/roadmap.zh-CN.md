...
- **运维可信度先于远程编排。** 在 E1 足够像一个可信生产观测中心之前，中心不应贸然长出 E2 作业派发能力：生产 / dev 传输边界要明确，部署与恢复要可复现，审计生命周期和 gap 要可见，节点认证要可治理，健康信号要能在仪表盘上看清。
...
服务于画像「派发可溯源」与铁律「中心不抬高本地上限 / 不执行原始命令」。这一阶段需要刻意加门槛：在 E1 运维成熟包可信、且 Scoot 侧无人值守 readonly clamp 落地之前，`/jobs/lease` 应保持为已鉴权但空派发的占位。
...
- **E2 前置条件必须清楚。** 队列语义、能力 / 标签匹配、`idem_key` 幂等、容量背压、deadline、`job_event` 处理、派发溯源，以及 Scoot 侧无人值守 readonly clamp 必须先形成一致设计，再暴露任何局部派发 UI 或 API。
...
- **E1 先具备生产可信度，再进入 E2。** 生产 / dev 传输行为明确，有部署与恢复指导，边缘端点限制可见，审计保留 / gap 行为运维者可理解，健康信号先在仪表盘上呈现，再扩大远程派发能力。
