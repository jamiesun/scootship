...
- **`scoot-edge` 是可选伴生程序，且可能滞后于中心契约。** Scoot 仓库现在已有 opt-in 的 E1 edge
  伴生程序和 E2 所需的无人值守 clamp，但完整派发上线仍依赖剩余 edge 侧 cwd confinement 以及命名的兼容契约。
  因此「能对着协议契约工作」「自带一个可模拟边缘上报与领取任务的测试装置（mock edge）」仍是 Scootship
  的硬需求，而不是锦上添花。
...
- **运维可信度先于远程编排。** 中心侧 E2 派发代码必须在明确门禁后生长：生产 / dev 传输边界要经过测试，部署与恢复要可复现，审计生命周期和 gap 要有实现，节点认证要可治理，健康信号要能在仪表盘上看清；在 edge 侧上线假设满足前，面向操作员的派发创建 / 控制入口仍保持关闭。只读派发审计可以展示 provenance，但不能新增权力面。
...
阶段二中心侧派发核心已落地，但面向操作员的派发上线仍需门禁：

- **持久化派发队列与 provenance。** append-only JSONL store 现在持久化派发任务快照，启动时可重放恢复，
...
- **只读派发审计。** 已登录的 dashboard/API 视图可以查看已持久化的派发任务、生命周期阶段、目标节点、
  请求 / 实际策略、幂等键、拒绝原因和 `session_id` 关联，但不暴露任何操作员派发表单或可写 API
  （`internal/center`、`internal/web`）。

仪表盘仍不暴露操作员派发表单或广域 fan-out 控制。完整 E2 上线仍需等待剩余的 Scoot edge 侧
`edge.job_root` cwd confinement 假设和操作员文档。
...
服务于画像「派发可溯源」与铁律「中心不抬高本地上限 / 不执行原始命令」。中心侧核心现在已经存在，但面向操作员的派发仍需要刻意加门槛，直到下方每一项上线条件都满足，包括剩余的 Scoot edge 侧 cwd confinement 假设。

E2 派发门禁对可写派发能力必须全满足。以下条件满足前，不得暴露局部派发创建 / 控制 UI/API、隐藏 feature
flag 或「仅管理员可用」绕行：
...
当前门禁状态：**中心侧核心已部分打开；操作员派发仍需门禁**。E1 侧现在已有直连 TLS、可信 TLS 反代 HTTP、显式 dev HTTP、默认明文 HTTP fail-closed 行为的代码与测试证据，也已有部署 / 恢复文档、审计保留与 gap 可见性、token 生命周期治理、运行审计时间线、只读健康信号、端点失败模式和严格遥测 body 校验的证据。Scoot `main` 现在已把无人值守一次性钳制（`scoot -e --unattended`）作为 E2 关键前置写入并实现；同时它的 EDGE.md 仍把 E2 上线门控在 `edge.job_root` cwd confinement。Scootship 现在已有派发队列、lease、幂等、能力不匹配拒绝、生命周期测试和只读派发审计面。剩余 edge 侧假设和操作员文档满足前，面向操作员的派发创建 / 控制 UI/API 仍被阻塞。
...
- **只降不升的策略表达。** 中心侧 enqueue 路径已实现：派发时只能请求 `≤` 节点本地上限的策略，默认
  `readonly`；中心 UI / API 不提供任何「抬高节点上限」的入口。
- **派发溯源审计。** 已作为只读审计面实现：中心侧记录派发 provenance，更新 lifecycle/session 关联，
  并把队列 / provenance 暴露给操作员查看；可通过 `session_id` 关联回摄入的 Scoot 运行审计，形成端到端可追溯链路。
- **E2 前置条件必须清楚。** 队列语义、能力 / 标签匹配、`idem_key` 幂等、capacity 限制、deadline、
  `job_event` 处理、只读派发溯源，以及 Scoot 侧无人值守 readonly clamp 现在已有代码、测试或上游契约证据；
  面向操作员的派发创建仍等待剩余 edge cwd confinement 和 runbook 工作。
