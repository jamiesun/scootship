...
> **状态：阶段二中心侧派发核心（pre-1.0）。**
> 中心摄入 `status` 心跳与 `audit_batch` 日志上报，并渲染车队。
> 任务**派发 / 编排（EDGE.md E2）**现在已有持久化中心侧队列、节点绑定的 lease 输出、生命周期摄入，
> 以及幂等 / provenance 测试。拥有 `dispatch:manage` 能力的仪表盘操作员可以创建新的、面向单个
> 节点的派发任务；`/dispatch` 列表本身仍是只读审计视图，任务控制（取消/重试）尚未构建。项目
> 形态、边界与方向见 [`roadmap.zh-CN.md`](roadmap.zh-CN.md)。
...
```text
   ...
   +----------------------^---------------------+
        ...
        +-----------------+-----------------+
        |                 |                 |
    scoot-edge        scoot-edge        scoot-edge   (可选伴生程序)
        |                 |                 |
      scoot             scoot             scoot
```
...
# 终端 2 —— 用内建模拟器跑通完整的 心跳 -> 摄入 -> 仪表盘 链路：
go run ./cmd/scootship mock-edge -ship-audit
```

打开 <http://localhost:8080>。你会被重定向到登录页 —— 在 dev 模式下用 `admin` / `admin` 登录。登录
后你会看到带**可收拢左侧菜单**的仪表盘外壳（Fleet、Dispatch、Tokens、Operators + 含 Account 的 Settings 菜单），
...
| `SCOOTSHIP_DISPATCH_QUEUE_LIMIT` | `200` | 单个节点同时最多允许存在多少个非终态（排队/已租约/运行中）派发任务。超过此上限的新建操作员派发任务会被拒绝。 |
...
- **E2 中心侧核心（已实现）：** `GET /jobs/lease` 会鉴权节点，要求匹配的 `node` 查询参数，限制
  `capacity`，并且只返回已经为该节点持久化的任务。派发记录以 append-only JSONL 快照保存，包含
  `idem_key` 去重、节点绑定、能力 / 标签不匹配拒绝（`no_matching_capability`）、按节点上报天花板做
  only-lower 策略钳制，以及来自已校验 `job_event` 遥测的生命周期更新。拥有 `dispatch:manage`
  能力的仪表盘操作员可以从 `/dispatch/new` 创建一个新的、面向单个节点的任务（会话 + CSRF
  保护，受可配置的每节点待处理任务队列上限约束）；`/dispatch` 列表和 JSON API 仍然保持只读审计
  视图，对已存在的任务仍然没有取消 / 重试 / 编辑的控制面。
...
| `internal/store` | append-only JSONL 车队存储，带幂等审计摄入、重放、可见审计保留缺口、保留窗口内运行时间线，以及中心侧派发队列 / provenance 记录。 |
...
| `internal/center` | HTTP 服务器、bearer + 登录会话鉴权、能力门禁、CSRF 校验、遥测摄入、节点绑定 lease 派发、只读健康 / 派发审计信号、仪表盘 + JSON API。 |
| `internal/web` | 嵌入式仪表盘模板与静态资源（`embed.FS`）。 |
| `internal/mockedge` | 模拟的 scoot-edge 节点，用于本地 heartbeat、audit 与 lease smoke。 |
