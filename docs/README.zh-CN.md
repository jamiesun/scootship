...
> 以及幂等 / provenance 测试。剩余 edge 侧上线门禁满足前，仪表盘只暴露只读 Dispatch 审计视图，
> 仍不暴露操作员派发表单。项目形态、
...
- **E2 中心侧核心（已实现）：** `GET /jobs/lease` 会鉴权节点，要求匹配的 `node` 查询参数，限制
  ...
  only-lower 策略钳制，以及来自已校验 `job_event` 遥测的生命周期更新。剩余 edge 上线门禁仍打开时，
  仪表盘会通过只读 Dispatch 审计视图和 JSON API 展示队列 / provenance，但不提供操作员派发表单。
